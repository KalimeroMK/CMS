...
class Utils
{
    ...
    public static function getSettings($key)
    {
        if ($key == config('constants.CATEGORY_CUSTOM_CSS')) {
            ...
            $settings_custom_css->custom_css = self::findOrTxt(
                config('constants.CATEGORY_CUSTOM_CSS'),
                config('constants.CATEGORY_CUSTOM_CSS')
            );
        ...
        if ($key == config('constants.CATEGORY_CUSTOM_JS')) {
            ...
            $settings_custom_js->custom_js = self::findOrTxt(
                config('constants.CATEGORY_CUSTOM_JS'),
                config('constants.CATEGORY_CUSTOM_JS')
            );
        ...
        if ($key == config('constants.CATEGORY_SOCIAL')) {
            ...
            $settings_social->twitter_handle = self::findOrString(
                config('constants.CATEGORY_SOCIAL'),
                'twitter_handle'
            );
            $settings_social->twitter_url = self::findOrString(config('constants.CATEGORY_SOCIAL'), 'twitter_url');
            $settings_social->google_plus_page_url = self::findOrString(
                config('constants.CATEGORY_SOCIAL'),
                'google_plus_page_url'
            );
            $settings_social->skype_username = self::findOrString(
                config('constants.CATEGORY_SOCIAL'),
                'skype_username'
            );
            $settings_social->youtube_channel_url = self::findOrString(
                config('constants.CATEGORY_SOCIAL'),
                'youtube_channel_url'
            );
            $settings_social->vimeo_channel_url = self::findOrString(
                config('constants.CATEGORY_SOCIAL'),
                'vimeo_channel_url'
            );
            ...
            $settings_social->sharethis_span_tags = self::findOrTxt(
                config('constants.CATEGORY_SOCIAL'),
                'sharethis_span_tags'
            );
            ...
            $settings_social->show_big_sharing = self::findOrCheck(
                config('constants.CATEGORY_SOCIAL'),
                'show_big_sharing'
            );
        ...
        if ($key == config('constants.CATEGORY_COMMENTS')) {
            ...
            $settings_comments->comment_system = self::findOrString(
                config('constants.CATEGORY_COMMENTS'),
                'comment_system'
            );
            $settings_comments->fb_js = self::findOrTxt(config('constants.CATEGORY_COMMENTS'), 'fb_js');
            $settings_comments->fb_num_posts = self::findOrString(
                config('constants.CATEGORY_COMMENTS'),
                'fb_num_posts'
            );
            $settings_comments->fb_comment_count = self::findOrCheck(
                config('constants.CATEGORY_COMMENTS'),
                'fb_comment_count'
            );
            $settings_comments->disqus_js = self::findOrTxt(config('constants.CATEGORY_COMMENTS'), 'disqus_js');
            $settings_comments->disqus_comment_count = self::findOrCheck(
                config('constants.CATEGORY_COMMENTS'),
                'disqus_comment_count'
            );
            $settings_comments->show_comment_box = self::findOrCheck(
                config('constants.CATEGORY_COMMENTS'),
                'show_comment_box'
            );
        ...
        if ($key == config('constants.CATEGORY_OLD_NEWS')) {
            ...
            $settings_old_news->auto_update = self::findOrCheck(
                config('constants.CATEGORY_OLD_NEWS'),
        ...
        if ($key == config('constants.CATEGORY_GENERAL')) {
            ...
            $settings_general->site_post_as_titles = self::findOrCheck(
                config('constants.CATEGORY_GENERAL'),
                'site_post_as_titles'
            );
            $settings_general->generate_sitemap = self::findOrCheck(
                config('constants.CATEGORY_GENERAL'),
                'generate_sitemap'
            );
            $settings_general->generate_rss_feeds = self::findOrCheck(
                config('constants.CATEGORY_GENERAL'),
                'generate_rss_feeds'
            );
            $settings_general->include_sources = self::findOrCheck(
                config('constants.CATEGORY_GENERAL'),
                'include_sources'
            );
    ...
    {
        return DB::table('settings')->where('category', $cat)->where('column_key', $col_key)->count(
        ) > 0 ? Setting::where('category', $cat)->where('column_key', $col_key)->first()->value_txt : '';
    }
    ...
    {
        return DB::table('settings')->where('category', $cat)->where('column_key', $col_key)->count(
        ) > 0 ? Setting::where('category', $cat)->where('column_key', $col_key)->first()->value_string : '';
    }
    ...
    {
        return DB::table('settings')->where('category', $cat)->where('column_key', $col_key)->count(
        ) > 0 ? Setting::where('category', $cat)->where('column_key', $col_key)->first()->value_check : 0;
    }
